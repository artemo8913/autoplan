...
export class PlanSerializationService {
    toDTO(meta: PlanMeta, stores: PlanEntityStores): PlanDTO {
        ...
        return {
            ...
            railway: {
                ...
                endX: railway.endX,
                ...(railway.picketage.length > 0
                    ? {
                          picketage: railway.picketage.map((e) => ({
                              km: e.km,
                              picketCount: e.picketCount,
                              ...(e.picketOverrides ? { picketOverrides: { ...e.picketOverrides } } : {}),
                          })),
                      }
                    : {}),
            },
            ...
            fixingPoints: stores.fixingPointsStore.list.map((fp) => ({
                ...
                supportType: fp.supportType === "pole" ? undefined : fp.supportType,
                crossSpanId: fp.crossSpan?.id,
            })),
    ...
    fromDTO(dto: PlanDTO, stores: PlanEntityStores): void {
        const rawRailway = dto.railway;
        const startX = rawRailway.startX;
        const endX = rawRailway.endX;
        const railway = new Railway({ name: dto.railway.name, startX, endX, picketage: rawRailway.picketage });
        ...
        // 5. CrossSpans (нужны до ТФ: ТФ под ригелем ссылается на поперечину) — зависят только от опор
        const crossSpansById = new Map<string, CrossSpan>();
        ...
        // 6. FixingPoints (resolve pole + track + crossSpan refs)
        ...
        const fixingPoints = dto.fixingPoints.map((d) => {
            ...
            const crossSpan = d.crossSpanId ? crossSpansById.get(d.crossSpanId) : undefined;
            const fp = new FixingPoint({
                id: d.id,
                pole,
                track,
                yOffset: d.yOffset,
                zigzagValue: d.zigzagValue,
    ...
    createEmptyDTO(name: string, startX = 0, endX = 10000): PlanDTO {
