...
export class CameraStore {
    ...
    /** Подогнать viewBox под диапазон участка */
    fitToRailway(startX: number, endX: number): void {
        const length = endX - startX;
        const padding = Math.max(length * 0.05, 100);
        this.viewBox = {
            x: startX - padding,
            y: -this.viewBox.height / 2,
            width: Math.min(length + 2 * padding, this.maxViewBoxWidth),
