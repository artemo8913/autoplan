...
const PolePreviewSymbol: React.FC<{ kind: string; consoleType?: string; vlType?: string }> = ({ kind, consoleType, vlType }) => {
    if (kind === "catenaryPole") {
        ...
        return (
            <g>
                ...
                {consoleLen > 0 && (
                    <line x1={0} y1={0} x2={consoleLen} y2={0} stroke="currentColor" strokeWidth={2} />
                )}
            </g>
        );
    }
    ...
    if (kind === "vlPole") {
        switch (vlType) {
        case "intermediate": return <circle cx={0} cy={0} r={6} fill="none" stroke="currentColor" strokeWidth={1.5} />;
        case "angular": return <polygon points="0,-7 6,5 -6,5" fill="none" stroke="currentColor" strokeWidth={1.5} />;
        case "terminal": return <rect x={-5} y={-5} width={10} height={10} fill="none" stroke="currentColor" strokeWidth={1.5} />;
        default: return <circle cx={0} cy={0} r={6} fill="none" stroke="currentColor" strokeWidth={1.5} />;
        }
    }
...
export const PlacementPreview: React.FC = observer(() => {
    ...
    if (toolState.tool !== "placement") return null;
    if (!toolState.previewPos) return null;
    ...
    return (
        <g className="placement-preview" style={{ pointerEvents: "none" }}>
            ...
            {/* Перекрестие */}
            <line x1={pos.x - 12} y1={pos.y} x2={pos.x + 12} y2={pos.y} stroke={color} strokeWidth={0.4} opacity={0.5} />
            <line x1={pos.x} y1={pos.y - 12} x2={pos.x} y2={pos.y + 12} stroke={color} strokeWidth={0.4} opacity={0.5} />

            {/* Label */}
            {labelText && (
                <g transform={`translate(${pos.x + 14}, ${pos.y - 20})`}>
                    <rect x={-2} y={-10} width={labelText.length * 5.5 + 8} height={16} rx={3} fill="white" stroke={color} strokeWidth={0.5} opacity={0.9} />
