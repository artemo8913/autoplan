...
function BulkPolesModalComponent() {
    ...
    const handleApply = useCallback(() => {
        const { valid, errors } = validateRows(rows, knownTrackIds, tracksStore.railway.picketage);
        if (errors.length > 0) {
        ...
        handleClose();
    }, [rows, knownTrackIds, entityService, handleClose, tracksStore]);
            ...
            <Table withTableBorder withColumnBorders>
                <Table.Thead>
                    <Table.Tr>
                        ...
                        <Table.Th w={70}>Км</Table.Th>
                        <Table.Th w={70}>Пк</Table.Th>
                        <Table.Th w={70}>М</Table.Th>
                        ...
                        <Table.Tr key={row.rowKey}>
                            ...
                            <Table.Td>
                                <TextInput
                                    size="xs"
                                    value={row.km}
                                    error={getError(idx, "km")}
                                    onChange={(e) => updateRow(row.rowKey, { km: e.target.value })}
                                />
                            </Table.Td>
                            <Table.Td>
                                <TextInput
                                    size="xs"
                                    value={row.pk}
                                    error={getError(idx, "pk")}
                                    onChange={(e) => updateRow(row.rowKey, { pk: e.target.value })}
                                />
                            </Table.Td>
                            <Table.Td>
                                <TextInput
                                    size="xs"
                                    value={row.m}
                                    error={getError(idx, "m")}
                                    onChange={(e) => updateRow(row.rowKey, { m: e.target.value })}
                                />
                            </Table.Td>
                            <Table.Td>
                                <Select
