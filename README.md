...
19. При нажатии на среднюю кнопки мышки последняя выбранная опора остается (и соответствующая панель) - это хорошо. Если поменять выбрать режим перемещения холста в тулбаре, то опора перестает быть выбранной, панель пропадает - это уже не удобно. Хочется, чтобы панель с редактированием опоры/опор была также в тулбаре (как и пути) и не исчезала. Думаю, даже если будет клик по пустой области, лучше чтобы последняя выделенная опора/объект не стирался. Тоже касается и выделения - есть выбранная опора, человек случайно выделил пустую область, всё слетело, надо заного выбирать опору.
20. В консоли появляются ошибки: "
    [Violation] 'mouseup' handler заняло <N> мс
    react-dom_client.js?v=b83cd2c6:13527 [Violation] 'mouseup' handler took 274ms
    react-dom_client.js?v=b83cd2c6:13527 [Violation] 'mouseup' handler took 153ms
    react-dom_client.js?v=b83cd2c6:13527 [Violation] 'mouseup' handler took 156ms
    react-dom_client.js?v=b83cd2c6:13527 [Violation] 'mouseup' handler took 150ms
    react-dom_client.js?v=b83cd2c6:13527 [Violation] 'mouseup' handler took 160ms
    react-dom_client.js?v=b83cd2c6:13527 [Violation] 'mouseup' handler took 242ms
    react-dom_client.js?v=b83cd2c6:13527 [Violation] 'mouseup' handler took 239ms
    react-dom_client.js?v=b83cd2c6:13527 [Violation] 'mouseup' handler took 257ms
    ". Но при этом мне кажется, что mousedown должен вызывать по-настоящему большую задержку.При клике HIt service начинает перебирать все объекты, а их может быть тысячи. Даже сейчас, когда их десяток, чувствуется задержка.
21. При выделении множества объектов рисуется сколько выбранно на данный момент объектов - я думаю это не надо. Скорее всего в коде при протягивании приходится перещитывать каждый раз вошел ли или не вошел объект в область выделения, а реализовано это сейчас сплошным перебором. Лучше уж чтобы при mouseup один раз перебор был совершен.
22. Множественное выделение - логично было бы, чтобы при нажатом shift выделенные опоры добавлялись
23. Возможность менять масштаб. Модалка с настройками визуального отображения
