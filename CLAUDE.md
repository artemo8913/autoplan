...
## План: Внедрение UI-инструментов редактора
Приложение сейчас отображает готовый план КС только для просмотра. Пан/зум реализован локальным состоянием в app/index.tsx. Выделение опор — через простейший UIStore с selectedPoleId. Нет инструментов создания/редактирования объектов.

Цель: добавить интерактивный редактор с инструментами (выделение, размещение опор, рисование проводов), state machine в UIStore, snap-привязку, overlay-слои, тулбар, строку подсказок. Реализация итерационная — каждый шаг компилируется и даёт рабочий результат.

Шаг 1 — Типы и новый UIStore
Цель: Компилируется без ошибок, визуально ничего не меняется (пан/зум временно не работает).

Шаг 2 — InteractiveCanvas + Pan/Zoom
Цель: Пан/зум переедет из локального useState в UIStore.viewBox. Canvas навигируем.

Шаг 3 — Выделение, Hover, Лассо
Цель: Клик на объект → синяя рамка. Shift+клик — мультивыбор. Drag из пустоты — лассо.

Шаг 4 — Инструмент размещения (Toolbar + PlacementPreview)
Цель: Кнопка тулбара → режим размещения → призрак → snap → клик → опора создана → Undo.

Шаг 5 — PropertyPanel (замена PoleEditorPanel)
Цель: Правая панель стала универсальной: КС-опора, ВЛ-опора, мультивыбор, режим размещения.

Шаг 6 — Delete, Undo/Redo
Цель: Delete удаляет выделенное; Ctrl+Z/Y работают для всех операций

Будущие шаги (не в текущем скоупе)
WireDrawing — handleWireDrawingMouseDown есть в InputHandler, нужен EntityOperations.createWireLine
CrossSpan — handleCrossSpanMouseDown есть, нужен EntityOperations.createCrossSpan
