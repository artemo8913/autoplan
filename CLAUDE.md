...
│   ├── poleEditor/
│   │   ├── index.ts
│   │   └── ui/PoleEditorPanel.tsx   # правая панель редактирования выбранной КС-опоры
│   ├── toolbar/
│   │   ├── index.ts
│   │   └── ui/Toolbar.tsx           # кнопки инструментов (placement, pan, select)
│   ├── placementPreview/
│   │   ├── index.ts
...
│   └── statusBar/
│       ├── index.ts
│       └── ui/StatusBar.tsx         # строка подсказок (statusHint + координаты)
│
└── shared/
...
- Тестовые данные в `src/app/initMock.ts::createTestData()` — вынести позже в JSON или отдельный источник.
- **FSD-исключение**: Layer-компоненты (`entities/catenaryPlanGraphic/ui/`) импортируют `useStore`/`useServices` из `@/app` — нарушение иерархии FSD (entities → app). Принято осознанно: компоненты тесно связаны с доменными моделями из того же entities-слайса, и выделение в отдельный features-слайс создало бы лишние косвенности. При пересмотре — перенести в `features/catenaryPlan/ui/`.
