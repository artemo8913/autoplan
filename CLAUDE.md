...
SVG-отрисовка, на данный момент бэкенда.
...
- **Кривые не реализуем** — пути отрисовываются прямыми.
...
- **Store** — все entity-сторы создаются в `compositionRoot.ts::init()`. Layer-компоненты зачастую не принимают пропсы, читают сторы через `useStore()`.
- **ID** — `crypto.randomUUID()`
- **SVG-компоненты** из `gost-symbols.tsx` — использовать как есть, символы могут уточняться по ходу вручную.
- **InputHandler** — координирует события мыши/клавиатуры → вызывает методы сервисов. Получает их через DI (конструктор). - **Сервисы** не используют MobX напрямую.
...
**FSD-исключение (принято осознанно):** импортируются типы, `useService` и `useStore` из `@/app` в слои нижнего уровня.

## Открытые вопросы моделирования

- **Сдвоенные опоры + ригель:** 2 стойки вплотную (один номер) с каждой стороны, ригель на 4 стойках. Три варианта: (1) `stanchionCount: 1|2` в `CatenaryPole` — просто, покрывает 95% случаев; (2) `PoleAssembly { stanchions[] }` — гибко, но усложняет модель; (3) 4 отдельные стойки + `poleGroupId` — избыточно. Решение отложено.
- **Анкер как отдельная сущность:** пока свойство опоры (`anchorGuy`/`anchorBrace`). Сценарий "демонтированный анкер" крайне редок — при необходимости `OrphanAnchor`.
...
- **Тестовые данные** в `src/app/initMock.ts::createTestData()` — вынести в отдельный модуль позже.
...
- Drag-and-drop выделенных опор + Shift для ограничения оси (новый ToolState `"drag"`, `BatchCommand` с позициями, `DRAG_THRESHOLD` уже есть в InputHandler)
- Inline click-to-edit: двойной клик на имя/зигзаг/длину пролёта → inline-редактор через абсолютно-позиционированный HTML (гибрид: одиночный клик = side panel, двойной = inline)
- Зигзаги: убедиться что `FixingPoint.zigzagValue` observable, добавить `setZigzagValue` action

**Фаза B — Улучшение размещения и workflow (P2):**

- "Магнетизм" оси габарита: snap к габариту соседних опор + пунктирная линия в `PlacementPreview`
- `DisplaySettingsStore` (MobX) — вынести масштабы/размеры из `constants.ts` в настраиваемый стор + модалка настроек
- Контекстное меню (ПКМ) — Mantine `Menu`, пункты зависят от контекста выделения
- Исправления рендеринга: стрелки зигзагов (длина + привязка к пути), аудит `getCatenaryPoses`

**Фаза C — Доменные фичи (P2 domain):**

- Workflow создания АУ/ИС (два подхода: wizard через toolbar + selection-based через ПКМ)
- Inline-редактирование длины пролёта (клик на число → NumberInput, Shift = сдвинуть цепочку)
- Массовое создание опор: модалка-таблица (name, x, track, gabarit, side)
- Средние анкеровки (Stage 7)
- Авто-оттяжка на границах АУ

**Фаза D — Вывод и данные:**

- Таблица-сетка и спецификация (Stage 9) — SVG-таблица над/под планом
- XLSX импорт опор (SheetJS)
- Редактирование параметров railway (км/пк)

**Фаза E — Рефакторинг и housekeeping (P3-P4):**

- Класс `Anchor { type, direction }` вместо полей `anchorGuy`/`anchorBrace`
- Профилирование рендеринга (React DevTools → per-entity observer-подкомпоненты)
- Сдвоенные опоры + ригель (открытый вопрос моделирования, см. README.md)
- Трансформаторы/разъединители (новые типы сущностей)
...
1. `gost-reference.md` — справочник символов ГОСТ
...
5. `src/app/store/ToolStateStore.ts` — state machine инструментов (ToolState discriminated union)
6. `src/app/services/InputHandler.ts` — обработчик ввода
7. `src/shared/types/toolTypes.ts` — `EntityType`
